/**
 * Processes database query results into standardized format
 * Transforms raw database rows into ClinVar-compatible structure
 */
exports.processDbResults = (results, searchTerm) => {
  ...
  exports.constructSearchQuery = (searchGroups) => {
	const queries = searchGroups.map(group => {
	  const conditions = [];
	  const params = [];
