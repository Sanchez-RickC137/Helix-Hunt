...
function parseDate(dateStr) {
  ...
  /**
   * Sorts assertions by classification date in descending order
   * @param {Object} a - First assertion to compare
   * @param {Object} b - Second assertion to compare
   * @returns {number} Comparison result for sorting
   */
  function sortByClassificationDate(a, b) {
	const dateA = parseDate(a.Classification.date);
	const dateB = parseDate(b.Classification.date);
  ...
  const processAndFilterResults = (results, filters) => {
	...
	return results.map(result => {
	  ...
	  // Sort filtered assertions by date before returning
	  filteredAssertions.sort(sortByClassificationDate);
